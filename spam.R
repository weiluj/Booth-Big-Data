...
email$spam <- factor(email$spam,levels=c(0,1),
    labels=c("important","spam"))
...
null_model<-glm(spam~1,data=email, family="binomial")
...
prop.table(table(email$spam, email$word_george),2)

table(email$spam, email$word_cs)

prop.table(table(email$spam, email$word_cs),2)
...
# predict spam v not for first 4 obsv
